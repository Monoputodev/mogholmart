...
$old_attributes = [];
$old_attributes_data = $data->relProductAttribute;
    ...
    <div class="col-md-10">
        ...
        if(count($attributes) > 0){
            foreach ($attributes as $attr){
                ...
                if(!empty($attr_item)){
                    ...
                    if(count($attr_options_data) > 0){
                        foreach ($attr_options_data as $option){
                            $attr_options[$option->frontend_title] = $option->frontend_title;
                        }
                    }
                }
                ...
                {!! Form::label($attr_item->code_column,$attr_item->frontend_title) !!}
                @if($attr_item->type == 'textarea')

                {!! Form::textarea('Attribute['.$attr_item->code_column.']', ($attr_val != '')?str_replace('==','',$attr_val):Input::old('Attribute['.$attr_item->code_column.']'),['id'=>$attr_item->code_column,'class' => 'form-control']) !!}


                @elseif($attr_item->type =='text')

                {!! Form::text('Attribute['.$attr_item->code_column.']',($attr_val != '')?str_replace('==','',$attr_val):Input::old('Attribute['.$attr_item->code_column.']'),['id'=>$attr_item->code_column,'class' => 'form-control']) !!}
                ...
                <div class="row">
                    <div class="container-fluid attribute_item_container_{{ $attr_item->id }}" >
                        ...
                        @foreach($attr_options as $opt_item_key => $opt_item_value)
                        <?php $attr_counter++; ?>
        ...
        <input type="submit" name="finish" class="btn btn-warning pull-right btn-sm font-10" value="Save & Finished">
